...
void sys_sigaction_return(struct Trapframe* utf,struct Trapframe* ktf) {
//	printk("sigaction return;\n");
	restoreSigContext(utf,ktf);
		
}

int sys_kill_sigaction(u_int envid, int sig) {
	struct Env* e;
	struct sigaction_queue* sigqueue;
	if (envid2env(envid,&e,0) != 0) {
		return -1;
	}
	sigqueue = &e->env_sigaction_queue;
/*	if (sigqueue->lpos == sigqueue->rpos) {
		return -1;
	}*/
	sigqueue->sigqueue[(sigqueue->rpos++) % MAX_SIGACTION] = sig;
//	printk("current rpos is %08x\n",sigqueue->rpos);
	return 0;
}


int sys_procmask_sigaction(int how, const sigset_t* set, sigset_t* oldset) {
	struct sigaction* tmp = &curenv->env_sigactions[0];
	if (!set) {
		return -1;
	}
	
	printk("set is %08x %08x\n",set->sig[0],set->sig[1]);
	if (oldset) {
		(*oldset).sig[0] = tmp->sa_mask.sig[0];
		(*oldset).sig[1] = tmp->sa_mask.sig[1];	
		printk("oldset is %08x %08x\n",tmp->sa_mask.sig[0],tmp->sa_mask.sig[1]);
	}
	switch(how) {
		case 0:
			for (int i = 1; i <= MAX_SIGACTION; ++i) {
				if (set->sig[(i - 1) >> 5] & (1 << (i - 1)) == 0) {
					tmp->sa_mask.sig[(i - 1) >> 5] &= ~(1 << (i - 1));
				}
			}
			
			//tmp->sa_mask.sig[0] |= set->sig[0];
			//tmp->sa_mask.sig[1] |= set->sig[1];
			break;
		case 1:
			for (int i = 1; i <= MAX_SIGACTION; ++i) {
				if (set->sig[(i - 1) >> 5] & (1 << (i - 1)) == (1 << (i - 1))) {
					tmp->sa_mask.sig[(i - 1) >> 5] |= (1 << (i - 1));
				}
			}
			//tmp->sa_mask.sig[0] &= ~set->sig[0];
...
int sys_signUp_sigaction(void (*do_sigaction)(void (*)(int),int,struct Trapframe*, struct Trapframe*),int signum, const struct sigaction * act, struct sigaction*oldact) {
	struct sigaction* tmp = &curenv->env_sigactions[signum];
	if (oldact) {
		(*oldact).sa_handler = tmp->sa_handler;
		(*oldact).sa_mask = tmp->sa_mask;
	}
	if (!act) {
		return -1;
	}
	curenv->env_sigaction_queue.do_sigaction = do_sigaction;
	tmp->sa_handler = act->sa_handler;
	tmp->sa_mask = act->sa_mask;
	return 0;
}
...
void *syscall_table[MAX_SYSNO] = {
	...
	[SYS_signUp_sigaction] = sys_signUp_sigaction,
	[SYS_procmask_sigaction] = sys_procmask_sigaction,
	[SYS_kill_sigaction] = sys_kill_sigaction,
	[SYS_sigaction_return] = sys_sigaction_return,
};
